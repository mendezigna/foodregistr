...
export class SignupPageComponent {
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.allFieldsAreOK()) {
      try {
        this.authService
          .signup(this.password, this.email, this.name)
          .then((res) => this.authService.deauthenticate());
      } catch (err) {
        switch (err.message) {
  ...
  private allFieldsAreOK(): boolean {
    return (
      !this.showNullEmailErr &&
      !this.showRepeatPassErr &&
      !this.showAlphaNumericErr &&
      !this.showShortPassErr &&
      !this.showNullNameErr &&
      !this.showMatchPassErr
    );
  }
  ...
  private verifyField(): void {
    ...
    this.showAlphaNumericErr =
      !/[a-zA-Z]/g.test(this.password) || !/[0-9]/g.test(this.password);
    this.showNullNameErr = this.name == "";
    this.showMatchPassErr =
      !this.showRepeatPassErr && this.repeatPassword != this.password;
  }
}
