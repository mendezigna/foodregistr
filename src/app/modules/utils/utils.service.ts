...
export class UtilsService {
  ...
  public stringToDate(dateString: string): Date {
    ...
    return new Date(
      splitedString[0] as unknown as number,
      (splitedString[1] as unknown as number) - 1,
      (splitedString[2] as unknown as number) || 1
    );
  }
  ...
  public getMonthDays(dateString: string): number[] {
    let date = new Date(
      dateString.split("-")[0] as unknown as number,
      (dateString.split("-")[1] as unknown as number) - 1,
      1
    );
    const days = [];
    while (
